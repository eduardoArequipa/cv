...
const ProjectCard = ({ title, category, description, image, tags }: Project) => (
  <motion.div 
    ...
    viewport={{ once: true }}
    className="group relative rounded-2xl overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-blue-500/50 transition-all duration-300 shadow-sm hover:shadow-xl"
  >
    ...
    <div className="absolute bottom-0 left-0 right-0 p-6 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {tags.map((tag: string) => (
          <span key={tag} className="px-2 py-1 text-xs font-medium bg-blue-500/20 text-blue-200 rounded-md border border-blue-500/20">
            {tag}
          </span>
        ))}
      </div>
